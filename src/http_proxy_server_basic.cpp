...
void http_proxy_server_basic::start_accept()
{
	...
	this->acceptor.async_accept(*socket, [socket, this](const error_code& error) {
		if (!error) {
			...
			auto connection = http_proxy_server_connection::create(std::move(*socket), std::move(asio::ip::tcp::socket(acceptor.get_io_service())), logger, http_proxy_server_config::get_instance().increase_connection_count());
