...
namespace azure_proxy
{
	...
	template<typename T>
	static bool http_proxy_client_config::init_cipher_for_connection(T& _connection)
	{
		...
		char cipher_code = 0;
		const auto& cipher_name = get_instance().get_cipher();
		if (cipher_name.size() > 7 && std::equal(cipher_name.begin(), cipher_name.begin() + 3, "aes"))
		{
			...
			aes_generator::generate(cipher_name, cipher_code, ivec, key_vec, _connection.encryptor, _connection.decryptor);
			std::copy(ivec.cbegin(), ivec.cend(), cipher_info_raw.begin() + 7);
			std::copy(key_vec.cbegin(), key_vec.cend(), cipher_info_raw.begin() + 23);
		}

		if (!_connection.encryptor || !_connection.decryptor)
		{
			return false;
		}
		...
		rsa rsa_pub(get_instance().get_rsa_public_key());
		if (rsa_pub.modulus_size() < 128)
		{
			_connection.logger->warn("{} invalid rsa public key", logger_prefix);
			return false;
		}

		_connection.encrypted_cipher_info.resize(rsa_pub.modulus_size());
		if (_connection.encrypted_cipher_info.size() != rsa_pub.encrypt(cipher_info_raw.size(), cipher_info_raw.data(), _connection.encrypted_cipher_info.data(), rsa_padding::pkcs1_oaep_padding))
		{
			_connection.logger->warn("{} invalid rsa encrypt size", logger_prefix);
			return false;
		}
		return true;
	}
