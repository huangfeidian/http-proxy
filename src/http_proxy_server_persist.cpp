...
void http_proxy_server_persist::start_accept()
{
	...
	this->acceptor.async_accept(*socket, [socket, this](const error_code& error) {
		if (!error) {
			...
			auto connection = http_proxy_server_session_manager::create(io_context, std::move(*socket), std::move(asio::ip::tcp::socket(this->acceptor.get_executor())), logger, http_proxy_server_config::get_instance().increase_connection_count());
