...
namespace azure_proxy
{
    ...
    :http_proxy_server_connection(std::move(_client_socket), std::move(_server_socket), in_connection_count), 
    _session_manager(in_session_manager)
    {
        
    }

    static std::shared_ptr<http_proxy_server_session> http_proxy_server_session::create(asio::ip::tcp::socket&& _client_socket, asio::ip::tcp::socket&& _server_socket,std::shared_ptr<spdlog::logger> logger, std::uint32_t in_connection_count, http_proxy_server_session_manager& in_session_manager)
    {
