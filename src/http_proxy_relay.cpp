	...
	void http_proxy_relay::start_accept()
	{
		...
		this->acceptor.async_accept(*socket, [socket, this](const error_code& error)
		{
			if (!error)
			{
				...
				auto connection = http_proxy_relay_connection::create(std::move(*socket), std::move(asio::ip::tcp::socket(this->acceptor.get_executor())), this->logger, http_proxy_relay_config::get_instance().increase_connection_count());
