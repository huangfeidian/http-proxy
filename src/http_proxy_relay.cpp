...
namespace http_proxy
{
	using std::cerr;
	using namespace std;
	http_proxy_relay::http_proxy_relay(asio::io_service& io_service) :
	...
	void http_proxy_relay::start_accept()
	{
		...
		this->acceptor.async_accept(*socket, [socket, this](const error_code& error)
		{
			if (!error)
			{
				...
				auto connection = http_proxy_relay_connection::create(std::move(*socket), std::move(asio::ip::tcp::socket(this->acceptor.get_io_service())), this->logger, http_proxy_relay_config::get_instance().increase_connection_count());
